...
`poi-search.js` 与 `searchPOI()` 调用 **One API** `POST /amap`（转发高德 v5/place），须：

```bash
export ONE_API_BASE=https://your-one-api-host:3000
export ONE_API_TOKEN=sk-xxxx
# 自签证书可选：export ONE_API_INSECURE=true
```

或在 `config.json` 中填写 `oneApiBase`、`oneApiToken`（可参考 `config.example.json`）。

### 路径规划、地理编码等（高德 Web Service Key）

以下脚本仍直连高德，需要 Web Service Key：

```bash
export AMAP_WEBSERVICE_KEY=your_key
# 或使用 config.json 的 webServiceKey
```

获取高德 Key：[高德开放平台](https://lbs.amap.com/api/webservice/create-project-and-key)

## 使用方法

### 1. POI 搜索

须已配置 `ONE_API_BASE` + `ONE_API_TOKEN`（见上文）。
...
async function travelExample() {
  ...
  console.log(result.mapTaskData);
}
