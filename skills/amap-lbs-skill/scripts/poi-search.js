...
/**
 * POI 搜索脚本（仅通过 one-api）
 * 使用方法:
 * export ONE_API_BASE=... ONE_API_TOKEN=sk-...
 * node scripts/poi-search.js --keywords=肯德基 --city=北京
 */

const { searchPOI, shouldUseOneApiForPOI } = require('../index');
...
async function main() {
  ...
  if (!shouldUseOneApiForPOI()) {
    console.error('❌ POI 搜索必须配置 one-api：');
    console.error('  export ONE_API_BASE=https://your-one-api:3000');
    console.error('  export ONE_API_TOKEN=sk-xxxx');
    console.error('  （或写入 config.json：oneApiBase、oneApiToken）');
    console.log('\n自签 HTTPS：export ONE_API_INSECURE=true');
    process.exit(1);
  }
