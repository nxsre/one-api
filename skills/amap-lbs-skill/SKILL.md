...
- 🔗 地图可视化链接生成
- 💾 配置本地持久化存储
- 🎯 POI 搜索经 **one-api** `POST /amap`（与 OpenAI 路径无关）；服务端持有高德 Key，客户端只需用户令牌

## 首次配置

### POI 搜索（必选）：**one-api**

`searchPOI`、`scripts/poi-search.js`、旅游规划中的 POI 拉取均调用 **One API** `POST /amap`，须：

```bash
export ONE_API_BASE=https://your-one-api-host:3000
export ONE_API_TOKEN=sk-xxxx
# 自签证书调试可选：
# export ONE_API_INSECURE=true
```

也可写入 `config.json`：`oneApiBase`、`oneApiToken`。

### 路径规划、地理编码等：高德 Web Service Key

场景二（链接拼接用到的地理编码）、场景五路径规划等仍直连高德，需：

1. [高德开放平台](https://lbs.amap.com/api/webservice/create-project-and-key) 创建应用并获取 Key  
2. `export AMAP_WEBSERVICE_KEY=your_key` 或 `export AMAP_KEY=your_key`，或 `config.json` 的 `webServiceKey`
...
通过 **one-api** `POST /amap`，在请求体中指定 `path` 为 `/v5/place/text` 或 `around` 等（与高德地点搜索 v5 一致）。须配置 `ONE_API_BASE`、`ONE_API_TOKEN`（或 `config.json`）。

### 执行步骤

1. 确认已导出 `ONE_API_BASE`、`ONE_API_TOKEN`（自签 HTTPS 时 `ONE_API_INSECURE=true`）
2. 调用 `scripts/poi-search.js` 或代码中的 `searchPOI()`

### 使用方法

```bash
export ONE_API_BASE=https://your-one-api:3000
export ONE_API_TOKEN=sk-xxxx
...
// 需先 export ONE_API_BASE / ONE_API_TOKEN，或使用 config.json 中 oneApiBase、oneApiToken
...
- 自动搜索指定城市的兴趣点（每类最多 5 个），POI 调用依赖 **one-api**（须 `ONE_API_BASE` + `ONE_API_TOKEN`）
- 按顺序规划各兴趣点之间的路线（路径规划段仍使用高德 Key）

---

## 配置管理

配置文件位于 `config.json`，示例见 `config.example.json`：

```json
{
  "webServiceKey": "路径规划、地理编码等非 POI 能力",
  "oneApiBase": "https://your-one-api:3000",
  "oneApiToken": "sk-xxxx"
}
```
...
1. **POI**：`ONE_API_BASE`、`ONE_API_TOKEN`（或 `oneApiBase` / `oneApiToken`）；自签 HTTPS：`ONE_API_INSECURE=true`
2. **路径规划等**：`AMAP_KEY` / `AMAP_WEBSERVICE_KEY` / `webServiceKey`
...
- 场景二、五的路径规划 / 地理编码等仍需要高德 Web Service Key；**POI 搜索仅走 one-api**，不再需要本机为 POI 配置 `AMAP_KEY`
