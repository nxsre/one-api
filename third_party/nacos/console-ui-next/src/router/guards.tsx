...
/** 同源 /nacos-ui/ 嵌入时跳转 One API /login，避免渲染控制台自带登录页 */
function OneApiLoginRedirect() {
  useEffect(() => {
    redirectToOneApiLoginPage();
  }, []);
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 via-sky-100 to-blue-100">
      <div className="animate-spin rounded-full h-8 w-8 border-2 border-sky-200 border-t-blue-600" />
    </div>
  );
}
