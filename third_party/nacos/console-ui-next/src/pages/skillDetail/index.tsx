...
export default function SkillDetailPage() {
  ...
  const handleCreateDraft = async (basedOnVersion?: string) => {
    ...
    let suggested = suggestNextVersionFromBase(basedOnVersion);
    try {
      const response = await skillApi.getVersion({
        namespaceId,
        skillName,
        version: basedOnVersion,
      });
      if (response && typeof response === 'object' && 'code' in response && response.code === 0) {
        const doc = response.data as SkillDocument | undefined;
        const declared = declaredVersionFromSkillDocument(doc);
        const bumped = declared ? suggestNextSemverPatchFromDeclared(declared) : null;
        if (bumped) {
          suggested = bumped;
        }
      }
    ...
    setCreateDraftTargetVersion(suggested);
...
function parseSemverLoose(version: string): { major: number; minor: number; patch: number } | null {
...
function declaredVersionFromSkillDocument(doc: SkillDocument | null | undefined): string | null {
  if (!doc?.resource) return null;
  const keys = Object.keys(doc.resource);
  const metaKey = keys.find((k) => k === '_meta.json' || k.endsWith('/_meta.json'));
  if (!metaKey) return null;
  const raw = doc.resource[metaKey]?.content?.trim() ?? '';
  if (!raw) return null;
  try {
    const j = JSON.parse(raw) as { version?: unknown };
    const v = typeof j.version === 'string' ? j.version.trim() : '';
    if (v && /^[A-Za-z0-9._~+\-]{1,64}$/.test(v)) return v;
  } catch {
    return null;
  }
  return null;
...
function suggestNextSemverPatchFromDeclared(declared: string): string | null {
  const s = parseSemver(declared) ?? parseSemverLoose(declared);
  if (!s) return null;
