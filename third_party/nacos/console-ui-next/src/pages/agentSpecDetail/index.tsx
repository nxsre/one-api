...
export default function AgentSpecDetailPage() {
  ...
  const handleCreateDraft = async (basedOnVersion?: string) => {
    ...
    try {
      const response = await agentSpecApi.getVersion({
        namespaceId,
        agentSpecName,
        version: basedOnVersion,
      });
      if (response && typeof response === 'object' && 'code' in response && response.code === 0) {
        const doc = response.data as AgentSpecDocument | undefined;
        const declared = doc?.content ? declaredVersionFromManifestContent(doc.content) : null;
...
function declaredVersionFromManifestContent(content: string): string | null {
