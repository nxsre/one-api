...
import ChannelModelMappingEditor from '../Channel/ChannelModelMappingEditor';
import FillCatalogModelsModal from '../Channel/FillCatalogModelsModal';
...
const TenantEditChannel = () => {
  ...
  const handleCancel = () => {
    navigate('/tenant-console/channels');
  };
  ...
  const loadChannel = async () => {
    ...
    if (fromState && String(fromState.id) === String(channelId)) {
      data = { ...fromState };
    } else {
      ...
      data = (rows || []).find((c) => String(c.id) === String(channelId));
      if (!data) {
  ...
  const fetchGroups = async () => {
    try {
      let res = await API.get(`/api/tenant_console/meta/groups`);
      setGroupOptions(
                ...
                <div className='channel-edit-models-toolbar'>
                  ...
                  <div className='channel-edit-model-test-concurrency'>
                    <label
                      ...
                      htmlFor='tenant-channel-test-models-concurrency'
                    >
                      {t('channel.edit.buttons.test_models_concurrency')}
                    </label>
                    <Input
                      id='tenant-channel-test-models-concurrency'
                      type='number'
                      min={1}
                      max={10}
                      step={1}
...
export default TenantEditChannel;
