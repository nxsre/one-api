...
const tokenId = computed(() => route.params.tid);
const isEdit = computed(() => tokenId.value !== undefined);

const base = computed(() => `/api/tenant_console/users/${encodeURIComponent(ownerId.value)}`);
const listPath = computed(() => `/tenant-console/users/${encodeURIComponent(ownerId.value)}/tokens`);
...
const loadModelsAndToken = async () => {
  ...
  try {
    const detailReq = API.get(`${base.value}/available_models_detail`);
    let parsedModels = [];
    if (isEdit.value) {
      const tr = await API.get(`${base.value}/tokens/${tokenId.value}`);
...
const submit = async () => {
  ...
  let res;
  if (isEdit.value) {
    res = await API.put(`${base.value}/tokens`, {
      ...localInputs,
      id: parseInt(tokenId.value, 10),
    });
  } else {
    res = await API.post(`${base.value}/tokens`, localInputs);
  }
