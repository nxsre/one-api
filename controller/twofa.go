...
func Setup2FA(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(c.Request.Context(), userId, model.LogTypeSystem, "开始设置两步验证")
...
func Enable2FA(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(c.Request.Context(), userId, model.LogTypeSystem, "成功启用两步验证")
...
func Disable2FA(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(c.Request.Context(), userId, model.LogTypeSystem, "禁用两步验证")
...
func RegenerateBackupCodes(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(c.Request.Context(), userId, model.LogTypeSystem, "重新生成两步验证备用码")
...
func Verify2FALogin(c *gin.Context) {
	...
	userId, ok := pendingUserIDFromSession(pendingUserId)
	if !ok {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "会话已过期或无效，请重新登录",
		})
		return
	}
	// 获取用户信息
...
func AdminDisable2FA(c *gin.Context) {
	...
	if myRole <= targetUser.Role && myRole != model.RoleRootUser {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	model.RecordLog(c.Request.Context(), userId, model.LogTypeManage,
		fmt.Sprintf("管理员强制禁用两步验证（操作者 id=%d username=%s）", adminId, adminName))
...
func pendingUserIDFromSession(v interface{}) (int, bool) {
	if v == nil {
		return 0, false
	}
	switch x := v.(type) {
