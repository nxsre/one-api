...
const (
	...
	anyfastRatioPresetID        = -102
	anyfastRatioPresetName      = "Anyfast 价格预设"
	anyfastRatioPresetBaseURL   = "https://www.anyfast.ai"
)
...
// upstreamPresetByID 将下拉列表中的负 ID 预设解析为可拉取的上游（不存数据库）。
func upstreamPresetByID(id int) (dto.UpstreamDTO, bool) {
	switch id {
	case officialRatioPresetID:
		return dto.UpstreamDTO{
	...
	case modelsDevPresetID:
		return dto.UpstreamDTO{
			...
			Endpoint: modelsDevPresetBaseURL + modelsDevPath,
		}, true
	case anyfastRatioPresetID:
		return dto.UpstreamDTO{
			ID:       anyfastRatioPresetID,
			Name:     anyfastRatioPresetName,
			BaseURL:  anyfastRatioPresetBaseURL,
			Endpoint: defaultEndpoint,
		}, true
	default:
		return dto.UpstreamDTO{}, false
	}
}

func getLocalPricingSyncData() map[string]any {
	data := billing_setting.GetPricingSyncData(map[string]any(ratio.GetExposedData()))
	data["image_ratio"] = ratio.GetImageRatioCopy()
	data["audio_ratio"] = ratio.GetAudioRatioCopy()
	data["audio_completion_ratio"] = ratio.GetAudioCompletionRatioCopy()
	return data
}

type upstreamChannelData struct {
	name string
	data map[string]any
}
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		logger.SysError("failed to bind upstream request: " + err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "请求参数格式错误"})
		return
	}
	if len(req.Upstreams) == 0 && len(req.ChannelIDs) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "请至少选择一个上游"})
		return
	}
	batch, err := model.CreateUpstreamSyncBatch(req.ChannelIDs)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": err.Error()})
		return
	}
	go runUpstreamSyncBatch(batch.Id, req)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "已开始后台拉取",
		"data": gin.H{
			"batch_id": batch.Id,
			"status":   model.UpstreamSyncStatusRunning,
		},
	})
}

func executeUpstreamRatioFetch(ctx context.Context, req dto.UpstreamRequest) ([]dto.TestResult, []upstreamChannelData, error) {
	if req.Timeout <= 0 {
	...
	if len(req.Upstreams) > 0 {
		...
		for _, id64 := range req.ChannelIDs {
			id := int(id64)
			if preset, ok := upstreamPresetByID(id); ok {
				upstreams = append(upstreams, preset)
				continue
			}
			if id > 0 {
		...
		if len(dbIds) > 0 {
			dbChannels, err := model.GetChannelsByIds(dbIds)
			if err != nil {
				logger.Error(ctx, "failed to query channels: "+err.Error())
				return nil, nil, fmt.Errorf("查询渠道失败")
			}
	...
	if len(upstreams) == 0 {
		return nil, nil, fmt.Errorf("无有效上游渠道")
	}
	...
	transport.DialContext = common.RatioSyncDialContext(dialer)
	httpClient := client.NewOutboundHTTPClientWithTransport(transport, 0)
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		if len(via) >= 10 {
			return fmt.Errorf("stopped after 10 redirects")
		}
		return common.ValidateOutboundURL(req.URL.String())
	}
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			if err != nil {
				logger.Warn(ctx, "build request failed: "+err.Error())
				ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
				return
			}

			if err := common.ValidateOutboundURL(fullURL); err != nil {
				logger.Warn(ctx, fmt.Sprintf("upstream ratio URL blocked (%s): %v", uniqueName, err))
				ch <- upstreamResult{Name: uniqueName, Err: fmt.Sprintf("请求地址未通过安全校验：%v", err)}
				return
			}
			...
			if isOpenRouter && chItem.ID != 0 {
				...
				key := ""
				for _, part := range strings.Split(dbCh.Key, "\n") {
			...
			if lastErr != nil {
				logger.Warn(ctx, "http error on "+chItem.Name+": "+lastErr.Error())
				ch <- upstreamResult{Name: uniqueName, Err: lastErr.Error()}
				return
			}
			defer resp.Body.Close()
			if resp.StatusCode != http.StatusOK {
				logger.Warn(ctx, "non-200 from "+chItem.Name+": "+resp.Status)
			...
			if ct := resp.Header.Get("Content-Type"); ct != "" && !strings.Contains(strings.ToLower(ct), "application/json") {
				logger.Warn(ctx, "unexpected content-type from "+chItem.Name+": "+ct)
			}
			limited := io.LimitReader(resp.Body, maxRatioConfigBytes)
			bodyBytes, err := io.ReadAll(limited)
			if err != nil {
				logger.Warn(ctx, "read response failed from "+chItem.Name+": "+err.Error())
				ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
				return
			}
			...
			if isOpenRouter {
				...
				if err != nil {
					logger.Warn(ctx, "OpenRouter parse failed from "+chItem.Name+": "+err.Error())
					ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
					return
				}
			...
			if isModelsDev {
				...
				if err != nil {
					logger.Warn(ctx, "models.dev parse failed from "+chItem.Name+": "+err.Error())
					ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
					return
				}
				ch <- upstreamResult{Name: uniqueName, Data: converted}
			...
			if err := common.DecodeJson(bytes.NewReader(bodyBytes), &body); err != nil {
				logger.Warn(ctx, "json decode failed from "+chItem.Name+": "+err.Error())
				ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
				return
			}
			...
			if err := common.Unmarshal(body.Data, &pricingItems); err != nil {
				logger.Warn(ctx, "unrecognized data format from "+chItem.Name+": "+err.Error())
	...
	var successfulChannels []upstreamChannelData
	...
	for r := range ch {
		if r.Err != "" {
			...
			successfulChannels = append(successfulChannels, upstreamChannelData{
				name: r.Name,
				data: r.Data,
			})
		}
	...
	if len(successfulChannels) == 0 {
		return testResults, nil, fmt.Errorf("全部上游拉取失败")
	}
	return testResults, successfulChannels, nil
}

func buildDifferences(localData map[string]any, successfulChannels []upstreamChannelData) map[string]map[string]dto.DifferenceItem {
...
func convertOpenRouterToRatioData(reader io.Reader) (map[string]any, error) {
	...
	for _, m := range orResp.Data {
		...
		ratio := promptPrice * 1000 * ratio.USD
...
func convertModelsDevToRatioData(reader io.Reader) (map[string]any, error) {
	...
	for modelName, candidate := range selectedCandidates {
		...
		modelRatio := candidate.Input * float64(ratio.USD) / modelsDevInputCostRatioBase
